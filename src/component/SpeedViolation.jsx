        ...
        <section className={"mt-10 px-3 text-sm md:text-lg md:flex"}>
          ...
          <section className={"opacity-80 rounded-lg bg-[#312C6C] text-white flex flex-col md:w-full"}>
            ...
            <section className={"px-4 mt-4 flex justify-between items-center"}>
              <article>
                <button className={"flex justify-center items-center bg-[#C63333] px-2 py-1.5 gap-x-1 rounded md:p-1.5 shadow-2xl shadow-indigo-500/40"}>
                  <AiFillDelete />
                  Delete
                </button>
              </article>
            ...
            <article className={"mx-4 my-3 rounded border border-gray-600 md:flex md:flex-wrap md:justify-between lg:flex lg:flex-nowrap"}>
              {violations.map((detail) => {
                const {id, name} = detail
                return (
                  <table key={id} className="m-4 border-separate border-spacing-2 border border-slate-500 flex flex-col">
                    <thead className={"flex justify-between items-center p-2 md:gap-x-3 lg:text-xs"}>
                    <div>{name}</div>
                    <div>
                      <FaAngleUp />
                      <FaAngleDown />
